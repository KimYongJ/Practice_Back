...
public class AccessTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain) throws ServletException, IOException {
        try {
            ...
            if (requestURI.startsWith("/api/user/items")) {
            ...
            if (tokenProvider.validateToken(response, accessToken)) {                                   // 토큰의 유효성을 검증한다.
                Authentication authentication = tokenProvider.getAuthentication(response, accessToken); // 사용자 인증 정보를 가져온다.
            ...
            response.sendError(HttpServletResponse.SC_UNAUTHORIZED, e.getMessage());//400에러
            //response.getWriter().write("error : " + e.getMessage()); // 응답 body에 데이타 싣는 예시
            //filterChain.doFilter() 을 실행하지 않으면 컨트롤러로 사용자 요청이 전달되지 않고, 프론트에 error로 전달된다.
        }
    }
}